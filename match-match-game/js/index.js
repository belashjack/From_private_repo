class Game {
    ...
    createCards() {
        ...
        const getRandomFrontSide = function getRandomFrontSide(frontSides) {
            const index = getRandomInt(0, frontSides.length - 1);
...
const addListeners = function addListeners() {
    const changeActivePattern = function changeActivePattern(event) {
        if (
            event.target.classList.contains('back-side-pattern')
            && !event.target.classList.contains('active-pattern')
        ) {
    ...
    const changeActiveDifficulty = function changeActiveDifficulty(event) {
    ...
    buttons.forEach((button) => {
        button.addEventListener('mousedown', forceBlur);
    });
